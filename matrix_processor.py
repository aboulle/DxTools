...
folder_path = "/home/aboulle/Dropbox/Boulot/ab_2018/Donnees/Ir-MgO/111/"
file_name = "Map311TOTAL-phi180_Q_matrix.txt"

th=3 # Integration width

rsm = np.loadtxt(folder_path+file_name)
...
def reset(event):
    ...
    global list_x, list_z, x, z, fit_matrix, peak_list, flag_fit, guess_list, p_fit, contour_list
    list_x = []
    list_z = []
    peak_list = []
    fit_matrix = np.zeros(np.shape(intensity))
...
def save_scans(event):
    outfile_name=file_name.split('.')[0]
    np.savetxt(folder_path+outfile_name+".xscan", np.column_stack((Qx,extract_hprofile(intensity, z,th))), fmt='%10.8f')
    np.savetxt(folder_path+outfile_name+".zscan", np.column_stack((Qz,extract_vprofile(intensity, x,th))), fmt='%10.8f')
...
buttonFit.on_clicked(fit2D)
buttonSelect.on_clicked(select_area)
fig.canvas.mpl_connect('key_press_event', key_press)#, lambda event: key_press(event, ax1))
